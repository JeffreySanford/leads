...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatButtonModule, MatCardModule, MatListModule, MatIconModule, MatProgressSpinnerModule, MatChipsModule, MatBadgeModule],
...
export class PackLeadsComponent implements OnInit {
  ...
  get filteredLeads(): LeadResponseDto[] {
    ...
    return this.leads
      .map(lead => {
        ...
        const nonSampleContracts = lead.contracts?.filter(c => !c.isSample) || [];
  ...
  get testCount(): number {
    return this.leads.filter(lead => 
      lead.contracts && lead.contracts.some(c => c.isTest)
    ).length;
  }
  ...
  toggleExpand(leadId: string): void {
    ...
    if (this.expandedLeads.has(leadId)) {
      ...
      console.log('Expanded:', leadId, 'Total expanded:', this.expandedLeads.size);
    }
  }
  ...
  packLeads() {
    this.loading = true;
    this.hasRun = true;
    this.http.get<{ leads: LeadResponseDto[]; scriptOutput: string }>('/api/pack').subscribe({
      next: (data) => {
  ...
  testLiveSam() {
    this.loading = true;
    this.http.get<{ success: boolean; message: string; contractsFound: number; contracts: unknown[] }>('/api/sam/test-live').subscribe({
      next: (data) => {
        console.log('🔴 LIVE SAM.gov API Response:', data);
        alert(`✅ SAM.gov API Test Complete!\n\nFound ${data.contractsFound} contracts under $250K with Small Business Set-Aside\n\nCheck console for full details.`);
        this.loading = false;
      },
      error: (err) => {
  ...
  searchNdIt() {
    this.loading = true;
    this.http.get<{ success: boolean; message: string; contractsFound: number; naicsCodesSearched: string[]; contracts: unknown[] }>('/api/sam/nd-it').subscribe({
      next: (data) => {
