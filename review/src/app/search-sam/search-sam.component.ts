...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatButtonModule, MatCardModule, MatFormFieldModule, MatInputModule, MatListModule, MatIconModule, MatProgressSpinnerModule, MatChipsModule, MatBadgeModule, FormsModule],
...
export class SearchSamComponent {
  ...
  search() {
    ...
    this.leads = [];
    this.searchTotal = 0;
    this.http.post<{ results: string[]; total: number; leads: LeadResponseDto[] }>('/api/search', { term: this.term }).subscribe({
      next: (data) => {
