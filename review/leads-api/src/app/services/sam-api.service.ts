...
export class SamApiService {
  ...
  async searchContracts(params: {
    ...
    try {
      ...
      const data = await response.json() as { opportunitiesData?: Record<string, unknown>[] };
      ...
      console.log('🔍 Before value filter:', opportunities.length, 'opportunities');
      
      if (params.maxValue) {
        const beforeCount = opportunities.length;
        opportunities = opportunities.filter((opp) => {
          const value = this.extractContractValue(opp as { award?: { amount?: string }; estimatedValue?: string; baseAndAllOptionsValue?: string });
          return value && value <= (params.maxValue || 0);
        });
        console.log(`💰 After $${params.maxValue} filter: ${opportunities.length} of ${beforeCount} opportunities`);
      }
  ...
  private extractContractValue(opportunity: { award?: { amount?: string }; estimatedValue?: string; baseAndAllOptionsValue?: string }): number | null {
    const value = 
    ...
    return [
      {
        ...
        naicsCode: '541512',
        description: 'TEST DATA from SAM.gov API (using mock due to API key): IT support services for small Army installation. Includes help desk, server maintenance, network administration. Small Business Set-Aside opportunity under $250K. This would be a real contract if API key was configured.',
